...
add_action( 'init', 'mos_courier_taxonomies', 0 );

// create two taxonomies, categories and tags for the post type "book"
function mos_courier_taxonomies() {
	...
	$sargs = array(
		...
		'query_var'         => true,
		'rewrite'           => array( 'slug' => 'order-category' ),
	);
	register_taxonomy( 'order-scategory', array( 'order' ), $sargs );
	...
	$args = array(
		...
		'query_var'             => true,
		'rewrite'               => array( 'slug' => 'order-tag' ),
	);

	register_taxonomy( 'order-tag', 'order', $args );
}